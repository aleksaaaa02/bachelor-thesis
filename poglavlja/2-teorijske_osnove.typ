...
Једна од кључних предности _Rust_-a је безбедност на нивоу компајлера, који спречава грешке попут двоструког ослобађања меморије или међусобног блокирања (енг. _deadlock_). Концепти као што су власништво, позајмљивање и животни век омогућавају креирање стабилних и поузданих система без додатних провера у време извршавања. Све ове функционалности обезбеђене су од стране _borrow checker_-а.

У развоју _RustyArena_ видео-игре коришћени су различити библиотечки сандуци:
- _bincode_ - ефикасна серијализација података, корисна за слање стања игра преко мреже @bincode.
...
- _SQLx_ - сандук који олакшава комуникацију са базом података @sqlx.
...
_Godot Engine_, понекада и само _Godot_, je софтвер отвореног кода (енг. _open-source_) који се користи за развој 2Д и 3Д игара и бесплатан је за коришћење. Познат је по својој једноставности и архитектури заснованој на сценама. Једна од његових највећих предности је сценски систем (енг. _scene tree_), који представља хијерархију објеката (чворова) који заједно граде свет игре. Сваки чвор има одређену улогу -- може представљати визуелни елемент, део корисничког интерфејса (UI, енг. _user interface_), физички објекат или извор звука. Оваква структура омогућава модуларност и поновну употребу компоненти, што знатно олакшава развој и проширивост игре @godot.

У контексту приказа света, _Godot_ комбинује сцене у једно стабло чворова које се извршава током игре. Свака сцена може да садржи друге сцене, што омогућава да сложене сцене разложимо на подскуп сцена.

Подразумевани скриптни језик у _Godot_-у је _GDScript_, који је синтаксно сличан Пајтон-у и намењен је брзом развоју логике. Ипак за пројекте који захтевају високе перформансе или ближи контакт са системским ресурсима, постоји могућност коришћења других програмских језика преко повезница (енг. _bindings_) система. _Godot_ је претежно писан у C++ језику, па су повезнице за овај програмски језик добро документоване и подржане, док за језике попут _Rust_-а и подршка и документација су знатно слабије развијене.
...
У почетку развоја мултиплејер видео-игара преовладавао је _peer-to-peer_ комуникациони модел. _Peer-to-peer_ модел представља децентрализован комуникациони модел, где сваки чвор у мрежи има једнака права, а не постоји специјални чвор. Ослања се на потпуно повезану мрежасту топологију (енг. _fully connected mesh topology_). Овај модел за комуникацију се још увек користи у појединим играма, пример су игре стратегије у реалном времену (RTS, енг. _real-time strategy_) @p2pspringnature. На слици @img:peer-to-peer је приказан изглед овакве архитектуре.
...
Модел клијент-сервер представља дистрибуирану структуру апликације која раздваја задатке или количину посла на пружаоца ресурса и потрошача @client-server. Слика @img:klijent-server илуструје овакву архитектуру.
...
Ауторитативан сервер (енг. _authoritative server_) представља централну компоненту која има потпуну контролу над светом игре и представља једини извор истине (енг. _single source of truth_). Свака акција клијента пролази кроз сервер који проверава њену исправност, након чега сервер ажурира стање @client-server2. 
...
Слика @img:apstraktna-arhitektura приказује архитектуру пројекта на високом нивоу апстракције која обухвата неколико кључних компоненти:

- Сервер за игру (енг. _game server_) -- имплементиран у _Rust_-у, задужен за управљање стањем света, обраду логике и слање UDP порука клијентима.
...
- Сервер за аутентификацију (енг. _authentication server_) -- независан модул који, верфикацијом играча пре приступа серверу, обезбеђује контролисан приступ и омогућава распоређивање играча у сесије.
...
Када сервер касније пошаље званично стање, клијент га упоређује с својом предикцијом и уколико постоји разлика врши интерполацију и корекцију. Наведена техника важи под претпоставком да ће команда послата серверу бити успешно извршена. Ово омогућава играчима да имају осећај тренутног одговора, чак и при већим кашњењима, али уводи додатан проблем ако корисник пошаље две команде једну за другом пре него што је добио потврду од сервера за прву команду. Ово може произвести ефекат да играч "скаче" током игре @csp. Слика @img:csp3 описује овај проблем.

#figure(image("../slike/CSP3.png", width: 70%), 
caption: [
  Проблем који настаје наивном имплементацијом @csp
])<img:csp3>
...
- Основне функционалности игре које обухватају:
  - кретање играча на мапи
  - могућност да корисник пуца на друге играче
  - систем животних поена
