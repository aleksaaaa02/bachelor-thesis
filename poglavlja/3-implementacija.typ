...
У овом поглављу описана је детаљнија имплементација игре. Архитектура и имплементација апликације зависе од самог домена, односно конкретне видео-игре која се имплементира и њеног гејмплеја (енг. _gameplay_). Ако посматрамо пример неке игре са картама попут игре UNO#footnote[https://www.unorules.com/], за овакав тип игре није нам критично време одзива сервера, односно ако одговор закасни пар секунди, играч се неће осетити оштећеним као да је игра намештена. Код других игара које захтевају брзо ажурирање стања света, попут игре _Valorant_#footnote[https://playvalorant.com/en-gb/], мало кашњење може произвести лоше корисничко искуство. Због тога је важан добар одабир протокола за комуникацију и планирање унапред, јер различити жанрови видео-игара захтевају другачију архитектуру система.

== Дељени код 

_Bincode_ и _serde_ су _Rust_ библиотечки сандуци који олакшавају серијализацију и десеријализацију _Rust_ типова, као и самостално дефинисаних структура и енумерација. Све заједничке структуре које се користе у комуникацији смештене су у пројекат _common_ како би смањили дуплирање кода. Све што се серијализује мора да имплементира одговарајуће _Rust_ особине (енг. _traits_) _Serialize_ и _Deserialize_. Пример дефинисаног модела је дат у листингу @list:packet-primer.
...
_Game server_ је ауторитативан сервер на коме се врши симулација света игре. При покретању, сервер отвара два сокета (енг. _socket_), један од њих је намењен за комуникацију преко TCP-a док је други намењен за комуникацију преко UDP-a. Комуникација преко мреже, односно операције читања и писања порука, могу бити дуге и блокирајуће. Ово се може решити ако сервер имплементирамо као вишенитни сервер (енг. _multi-threaded_). Коришћењем _Tokio_ сандука можемо раздвојити обраду порука у одвојене асинхроне задатке. Конкретни асинхрони задаци подразумевају:
...
Обрада TCP захтева обухвата пријем порука, обраду и по потреби прослеђивање ка асинхроном задатку који симулира игру коришћењем канала (енг. _channel_). У тренутној имплементацији се овај механизам користи како би играч обавестио сервер да жели да приступи игри, али је могуће додати нове функционалност које захтевају поузданост.

Обрада долазних UDP захтева слична је обради TCP захтева. Долазне поруке су дефинисане у пројекту _common_, које се путем канала за асинхрону комуникацију прослеђују даље задатку која симулира игру. Ове поруке су заправо команде које играчи шаљу како би мењали стање света. Пре него што дође до промене стања, команда се валидира како бисмо спречили злонамерне кориснике да варају у играма и заобилазе правила игре. На листингу @list:komanda приказан је модел података команде која се шаље када корисник притисне одговарајући тастер.
...
Иако корисник приликом првог приступа серверу шаље TCP поруке, играч мора послати још једну поруку преко UDP конекције како би сервер био обавештен на коју адресу (IP и порт) треба да шаље стање света како би играчи имали ажуран приказ игре. Приликом прве команде, клијент јавља серверу да је спреман да приказује игру кориснику  и на коју адресу треба да га обавештава о стању игре. За поступак обавештавања на серверу задужен је _broadcasting_ асинхрони задатак. Он периодично емитује стање света играчима који су забележени као активни када асинхрони задатак који симулира свет то затражи.

=== _Game loop_

_Game loop_ је бесконачна петља која симулира свет и правила игре. Она ради фиксном фреквенцијом, која одређује број извршених итерација у једној секунди. Често се узима вредност од 60 Hz, што значи да једна итерација траје око 16 ms. Да бисмо осигурали да се симулација не извршава брже, проверавамо време између итерација петље и, по потреби, успављујемо нит на преостали део периода. Псеудо код који представља овакав алгоритам дат је испод у листингу @list:gameloop-code.
...
Осим кретања, играч има могућност да пуца како би уништио бродове других играча. Када играч притисне тастер за пуцање, сервер на основу координата и ротације брода рачуна правац и брзину кретања метка по _x_ и _y_ оси. Поред ових параметара бележи се и пређени пут метка како бисмо га након неког времена уништили, као и време последње испаљеног метка, како бисмо ограничили учесталост пуцања.

Поред метака и бродова као играча, имамо и астероиде. Астероиди уводе ефекат насумичности који повећава изазов игре како игра не би брзо досадила. Иницијализују се на следећи начин:
- На случајан начин се бира једна ивица света на којој ће се астероид иницијализовати.
- Након што је ивица одабрана, на случајан начин бира се положај на тој ивици и додаје се одређена маргина у случају да се неки играч нађе на ивици.
- За правац кретања астероида се прво на случајан начин бира неко од играча, након чега се астероиду додељује брзина кретања која се добија тако што се прво пронађе јединични вектор између играча и почетне позиције астероида након чега се множи са брзином кретања. 
Као и код испаљених метака постоји пређени пут који се рачуна при свакој итерацији чија вредност се користи за уклањање астероида који прелазе неку границу. Само кретање испаљених метака и астероида је једноставније, односно не постоји скалирање брзине током времена како бисмо симулирали успоравање.
...
- Ако је метак погодио неког играча, метак се уништава док се погођеном играчу одузимају животни поени. Конкретно се одузима 20 поена, али је ову вредност могуће конфигурисати.
- Ако је астероид погодио неког играча, астероид се уништава, а играчу се смањује број животних поена за одређену суму.
- Ако неко од активних играча има 0 животних поена, сматра се да је он уништен и иницијализује се на нову позицију.
- Ако постоје услови иницијализују астероиде. Конкретно астероиде инцијализујемо након сваких 1000 ms и по потреби се може прилагодити интервал између инстанцирања.
...
Особине које су наведене у исечку кода означавају да дата структура наслеђује одређену _Godot_ класу. Иако _Rust_ није објектно оријентисан језик, на овај начин се симулира наслеђивање. Поред дефинисања структура, може се имплементирати интерфејс који наслеђена класа поседује. Ово је некада обавезно, а посебно `init` метода која се користи за иницијализацију чвора. Пример је дат у листингу @list:interfejs.
...
- _process_ - функција се позива сваки пут када се фрејм (енг. _frame_) исцртава, чија учесталост зависи од брзине процесора.
- _physics_process_ - функција се позива на фиксан период, погодан за обраду и симулацију физике.

Поред имплементације интерфејса за одређену _Godot_ класу, могу се дефинисати и додатне методе унутар имплементационог блока дефинисане структуре.

=== Мрежна комуникација

_RustyArena_ поседује две структуре чији је фокус комуникација са серверима. Прва структура је _NetworkClient_ који се користи за комуникацију са сервером на коме се игра симулира. При покретању игре учитавају се адресе на којима се налазе сервери. Код у листингу @list:serverkonekcija представља имплементацију отварања сокета за UDP комуникацију. За адресу наведена је локална адреса (0.0.0.0), док је за порт постављена нула што означава коришћење било ког слободног порта.
...
pub fn connect_to_server(&mut self) {
    ...
    let socket = AsyncRuntime::block_on(sock_future); // чекање на одговор да ли је сокет успешно отворен
}
...
```, caption: [
...
```, caption: [
    Код који обавља слање команде
])<list:slanje_komande>

Имплементација функције која шаље иницијалну поруку преко TCP конекције је дата у следећем исечку кода. Сервер као одговор шаље исти идентификатор који је клијент послао као потврду да је сервер постао свестан новог играча.
...
Друга структура, _NetworkAPI_, се фокусира на комуникацију са сервером за аутентификацију. Методе која ова структура поседује су `login` и `register` које се баве пријавом и регистрацијом на систем. За комуникацију између чворова, _Godot_ поседује посебан механизам сигнала. Овај механизам заправо омогућава имплементацију _observer_ обрасца. Коришћењем _reqwest_ сандука шаље се захтев аутентификационом сервер и чека се одговор у посебном асинхроном задатку. Када одговор пристигне шаље се `process` методи _NetworkAPI_ чвора коришћењем асинхроних канала за комуникацију. Затим се проверава тип одговора који је пристигао, и емитује се одговарајући сигнал. Сви елементи, функције које су претплаћене и слушају задати сигнал, ће се извршити. На овај начин можемо обезбедити ефикасну асинхрону комуникацију без блокирања апликације. 

Разлог за раздвајање на две структуре је олакшана одрживост, ако је потребно проширити једну страну.
...
Клијенту је доступно стање света на основу кога он може да закључи да ли се његова команда извршила или приметити да је та команда донела промене. У овој игри, свака команда коју клијент шаље серверу садржи редни број команде који се инкрементује за сваку наредну команду и означава редослед њеног слања. Иако не враћа одговор на команду, сервер за сваког играча чува информацију о последњој обрађеној команди.

Када клијент добије ново стање света од сервера, он  проверава у својој локалној историји команди која је последња команда коју је сервер обрадио. Све команде које су послате пре тог тренутка се одбацују, док се све накнадне команде поново примењују. На овај начин клијент коригује своју предикцију и остаје усклађен са серверским стањем, без потребе за експлицитним потврдама након сваке акције.
...
pub fn apply_local_input(&mut self, action: InputAction, delta: f64) {
    match action {
        // за акције корисника дефинишемо локално понашање, због једноставности користићемо имплементацију сличну серверској
    }
...
Сервер за аутентификацију имплементиран је коришћењем _Tokio_ _Axum_ и _SQLx_ сандука. Његова главна сврха је да омогући корисницима приступ систему путем регистрације и пријаве, пре него што започну игру. 

Кориснички подаци се чувају у _PostgreSQL_ бази података, при чему се лозинке хешују (енг. _hashing_) како би се повећала безбедност. При свакој успешној пријави, сервер враћа идентификатор, који клијент касније користи за идентификацију током комуникације са сервером игре. Овај приступ је поједностављен и погодан за развојну фазу система, јер омогућава једноставну интеграцију са клијентом.
